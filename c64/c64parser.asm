...
remove_articles
		...
		sta $tableAddr
		lda #kbBufHi ;  set up addr  in $$kbdbuf
		sta $tableAddr+1		
		ldy #0
...
get_verb
		...
		pha
		ldy #0
_lp1	lda $$kbdbuf,y			;copy 1st word to word1
		sta word1,y
		iny
		cmp #0
		beq _shft1
...
_shft1	sty $wrdEnd			; shift keyboard buffer left	
		...
		stx $wrdEnd
_lp2	lda $$kbdbuf,x			;find end of 2nd word
		cmp #32 ; space
		beq _out
		sta word1,y
		inc $wrdEnd
...
_out	pha   ; save whitespace char
		lda #0
		sta $$kbdbuf,x  ; null terminate the 2nd word
		jsr is_preposition ; (uses tableAddr as src)
		pla 		; replace whitespace char
		sta $$kbdbuf,x  ; replace whitespace char
		lda $strIndex
		cmp #255
...
_lp		lda $$kbdbuf
		...
		jsr mov_to_end_of_first_word
		ldy $wrdEnd
		lda $$kbdbuf,y ; get and save 
		pha 
		lda #0
		sta $$kbdbuf,y ; null terminate word
		jsr is_preposition
		pla
		sta $$kbdbuf,y ; restore null/white space'
		lda $strIndex
		cmp #255			
